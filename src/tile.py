...
class TileMap:
    ...
    def find_neighbour_tiles(self, pos: tuple) -> List[dict]:
    ...
    def collision_tile_rects(self, pos: tuple) -> List[pygame.Rect]:
        ...
        for neighbour_tile in self.find_neighbour_tiles(pos):
            if neighbour_tile["type"] in ("grass", "stone"):
                tile_rects.append(
                    pygame.Rect(
                        neighbour_tile["pos"][0]
                        * self.tile_size,  # each tile takes its pixel position
